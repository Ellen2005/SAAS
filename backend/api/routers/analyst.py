...
class AnalyzeRequest(BaseModel):
    question: str = Field(..., min_length=1, description="The analysis question")
    data: list = Field(default_factory=list, description="Optional data rows")
    columns: list = Field(default_factory=list, description="Optional column names")
    method: str = "auto"


class StatsRequest(BaseModel):
    values: list = Field(default_factory=list, description="Values to analyze")
...
def get_augmented_insights(user_id: str = Depends(resolve_user_id)):
    ...
    try:
        ...
        for ins in insights:
            if ins.get("type") in ("trend_shift",):
...
def compute_statistics(
    ...
    try:
        ...
        if not body.values:
            return {"error": "No values provided for statistical analysis."}
...
def get_context(
    ...
    try:
        ...
        return {"history": [], "last_question": None, "active_filters": {}}
...
def run_full_analysis(context: dict = Depends(require_role(["manager", "admin"]))):
    ...
    try:
        ...
        df_rows = [{"date": r.get("recorded_at"), "kpi_name": r.get("kpi_name"),
                    "value": _to_float(r.get("value")), "department_id": r.get("department_id")} for r in kpi_rows]
        df = pd.DataFrame(df_rows)
        if "date" in df.columns:
            df["date"] = pd.to_datetime(df["date"], errors="coerce")
