...
REVENUE_KPI_NAMES = {"net_revenue", "Total Revenue", "total_revenue"}
...
def get_admin_summary(context: dict = Depends(require_role(["admin"]))):
    ...
    try:
        ...
        for row in kpi_rows:
            ...
            if kpi_name in REVENUE_KPI_NAMES:
                ...
                timeline_map[recorded_at]["department_breakdown"][dept_name] = float(
                    row.get("value", 0)
                )
