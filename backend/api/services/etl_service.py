...
def build_mock_frame() -> pd.DataFrame:
    ...
    data = []
    base_revenue, base_inventory, base_tickets = 135000, 450000, 89
    for i, day in enumerate(dates):
        revenue = 190000 if i == 29 else base_revenue + np.random.normal(0, 5000)
        inventory = base_inventory + np.random.normal(0, 8000)
        tickets = 150 if i == 29 else max(10, int(base_tickets + np.random.normal(0, 15)))
        data.extend([
            {"date": day, "kpi_name": "Total Revenue", "value": revenue,
             "source_row_id": str(uuid.uuid4()), "record_label": f"Invoice Batch {i+1}", "customer_id": f"ACCT-{i+1:04d}"},
            {"date": day, "kpi_name": "Inventory Value", "value": inventory,
             "source_row_id": str(uuid.uuid4()), "record_label": f"Inventory Snapshot {i+1}", "customer_id": f"ACCT-{i+1:04d}"},
            {"date": day, "kpi_name": "Support Tickets", "value": tickets,
             "source_row_id": str(uuid.uuid4()), "record_label": f"Ticket Batch {i+1}", "customer_id": f"ACCT-{i+1:04d}"},
        ])
...
def detect_anomalies_and_transform(df: pd.DataFrame):
    ...
    Compares today's value against the same-day-of-week rolling mean/std
    to avoid false positives from weekly seasonality patterns.
    Falls back to global 30-day stats if fewer than 4 same-weekday samples exist.
    """
    kpis = []
    anomalies = []
    ...
    for raw_kpi_name in df["kpi_name"].unique():
        ...
        # Day-of-week corrected baseline
        same_dow = historical[pd.to_datetime(historical["date"]).dt.dayofweek == today_weekday]
        if len(same_dow) >= 4:
            mean_baseline = same_dow["value"].mean()
            std_baseline = same_dow["value"].std()
        else:
            # Fallback to global 30-day stats
            mean_baseline = historical["value"].mean()
            std_baseline = historical["value"].std()
        ...
        dod_pct = ((current_val - yesterday_val) / yesterday_val * 100) if yesterday_val else 0
        wow_pct = ((current_val - last_week_val) / last_week_val * 100) if last_week_val else 0
        z_score = abs(current_val - mean_baseline) / std_baseline if std_baseline > 0 else 0
        ...
        kpis.append({
            ...
            "status": status,
            "recorded_at": today["date"].date().isoformat() if hasattr(today["date"], "date") else str(today["date"])[:10],
        })
...
def get_required_fields(user_id: str, supabase) -> list:
    try:
        ...
        if not department_id:
            return []
        dept_resp = supabase.table("departments").select("template_id").eq("id", department_id).limit(1).execute()
        template_id = dept_resp.data[0].get("template_id") if hasattr(dept_resp, "data") and dept_resp.data else None
        if not template_id:
            return []
        fields_resp = supabase.table("semantic_fields").select("global_field_name, data_type, required").eq("template_id", template_id).execute()
...
def run_user_etl_pipeline(user_id: str):
    ...
    try:
        ...
        print(f"[{datetime.now().isoformat()}] Extraction complete. {len(raw_df)} rows retrieved.")
        ...
        validation_results = run_all_validations(
            current_validation_df, required_fields,
        ...
        metric_counts = raw_df["kpi_name"].map(KPI_NAME_MAP).value_counts(dropna=True).to_dict()
