    ...
    <div className="landing-container" style={{ background: 'var(--bg-color)', minHeight: '100vh', color: 'var(--text-primary)', overflowX: 'hidden' }}>
      ...
      <section style={{ padding: '100px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div className="dashboard-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
          ...
          <div className="glass-panel" style={{ textAlign: 'left' }}>
            <div style={{ background: 'rgba(16, 185, 129, 0.1)', padding: '12px', borderRadius: '12px', width: 'fit-content', marginBottom: '20px' }}><Shield color="var(--status-normal)" /></div>
            <h3>{t('landing_feature2_title')}</h3>
            <p>{t('landing_feature2_desc')}</p>
          </div>
          <div className="glass-panel" style={{ textAlign: 'left' }}>
            <div style={{ background: 'rgba(245, 158, 11, 0.1)', padding: '12px', borderRadius: '12px', width: 'fit-content', marginBottom: '20px' }}><Database color="var(--status-warning)" /></div>
      ...
      <style>{`
        ...
        @keyframes floatHero { 0% { transform: translateY(0px) rotate(0deg); } 33% { transform: translateY(-10px) rotate(0.5deg); } 66% { transform: translateY(-5px) rotate(-0.5deg); } 100% { transform: translateY(0px) rotate(0deg); } }
        @keyframes slideUp { from { transform: translateY(100px); opacity: 0; } to { transform: translateY(0px); opacity: 1; } }
        .hero-image-container:hover { transform: scale(1.02) translateY(-15px) !important; box-shadow: 0 40px 80px -20px rgba(59, 130, 246, 0.4) !important; border-color: var(--primary-color) !important; }
      `}</style>
    </div>
  );
};
